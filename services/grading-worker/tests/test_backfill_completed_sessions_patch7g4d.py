...
# Add scripts/ so we can import backfill helpers directly.
_GRADING_WORKER_ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(_GRADING_WORKER_ROOT / "scripts"))

from backfill_completed_sessions import _build_candidate_sql, run  # noqa: E402
...
def _args(
    ...
    return argparse.Namespace(
        ...
        limit=limit,
        include_empty_raw=include_empty_raw,
        require_user_turn=require_user_turn,
        since=since,
...
class _Row:
    """Minimal asyncpg-like record for backfill tests."""
...
def _run_backfill(args: argparse.Namespace, rows: list, mock_job: AsyncMock) -> int:
    """Run backfill against fake rows with mocked DB and process_session_completed_job."""
    mock_conn = MagicMock()
    mock_conn.fetch = AsyncMock(return_value=rows)
    mock_conn.close = AsyncMock()

    with patch("backfill_completed_sessions.asyncpg.connect", AsyncMock(return_value=mock_conn)), \
         patch("backfill_completed_sessions.GradingRepository"), \
         patch("backfill_completed_sessions.process_session_completed_job", mock_job), \
         patch.dict("os.environ", {"DATABASE_URL": "postgresql://fake/testdb"}):
        return asyncio.run(run(args))


def test_candidate_sql_includes_failed_rows_for_manual_recovery() -> None:
    sql, params = _build_candidate_sql(
        since=None,
        session_id=None,
        include_empty_raw=False,
        limit=50,
    )

    assert "(gr.session_id IS NULL OR gr.status = 'failed')" in sql
...
def test_execute_skips_no_raw_backup():
    mock_job = AsyncMock()
    _run_backfill(_args(execute=True), [_Row(None)], mock_job)
    assert mock_job.call_count == 0


def test_execute_skips_invalid_raw_backup():
    mock_job = AsyncMock()
    _run_backfill(_args(execute=True), [_Row("not-valid-json")], mock_job)
    assert mock_job.call_count == 0
...
def test_execute_skips_no_user_turns():
    ...
    mock_job = AsyncMock()
    _run_backfill(_args(execute=True), [_Row(raw)], mock_job)
    assert mock_job.call_count == 0
...
def test_execute_skips_insufficient_words():
    ...
    mock_job = AsyncMock()
    _run_backfill(_args(execute=True, min_student_words=25), [_Row(raw)], mock_job)
    assert mock_job.call_count == 0
...
def test_dryrun_never_calls_job_for_eligible():
    ...
    mock_job = AsyncMock()
    _run_backfill(_args(execute=False, min_student_words=25), [_Row(raw)], mock_job)
    assert mock_job.call_count == 0
...
def test_execute_summary_counts_mixed_candidates(capsys):
    ...
    mock_job = AsyncMock()
    _run_backfill(_args(execute=True, min_student_words=25), candidates, mock_job)
...
def test_execute_min_words_high_threshold_skips():
    ...
    mock_job = AsyncMock()
    _run_backfill(_args(execute=True, min_student_words=50), [_Row(raw)], mock_job)
    assert mock_job.call_count == 0
...
def test_execute_min_words_low_threshold_allows():
    ...
    mock_job = AsyncMock()
    _run_backfill(_args(execute=True, min_student_words=5), [_Row(raw)], mock_job)
    assert mock_job.call_count == 1


# ---------------------------------------------------------------------------
# --include-empty-raw: NULL raw still gated by eligibility helper
# ---------------------------------------------------------------------------

def test_include_empty_raw_null_still_skipped():
    # Even with include_empty_raw=True, evaluate_grading_eligibility(None) → no_raw_backup
    mock_job = AsyncMock()
    _run_backfill(_args(execute=True, include_empty_raw=True), [_Row(None)], mock_job)
    assert mock_job.call_count == 0


# ---------------------------------------------------------------------------
# No transcript text leakage in output (capsys, targeted)
# ---------------------------------------------------------------------------

def test_no_transcript_leakage_in_output(capsys):
    secret_word = "supersecretword"
    raw = json.dumps([{"type": "USER_TURN", "payload": {"text": f"{secret_word} " * 30}}])
    mock_job = AsyncMock()
    _run_backfill(_args(execute=True, min_student_words=25), [_Row(raw)], mock_job)
