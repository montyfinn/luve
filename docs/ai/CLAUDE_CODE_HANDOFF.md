...
- **Per-session backpressure.** `media/coordinator.py` holds a bounded per-session `asyncio.Queue(maxsize=1)` drained by one `_inference_loop`. Drop/priority policy in `_enqueue_inference_job`: when the queue is full a **partial is dropped**; a **final evicts the queued partial and is prioritized**. Memory is bounded per session; partials degrade first under load.
...
- **Known limitations / non-goals (current):** no multi-session scheduler (single-session by design); **no per-inference timeout** (a hung `to_thread` transcription would hold `_inference_lock`; `asyncio.to_thread` is not cancellable, so `stop()` waits ≈ one inference); **no hard cap on final-utterance length** (partials are windowed, finals are not); no GPU tests in CI. These residual improvements live in hot-path files (`stt_worker.py`, `coordinator.py`, `luve_extension.py`) and should wait for a clean hot-path baseline before any code change.
